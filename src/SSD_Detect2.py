...
ssd_detect2_spec = ["implementation_id", "SSD_Detect2",
		 "type_name",         "SSD_Detect2",
...
#
#
class SSD_Detect2(OpenRTM_aist.DataFlowComponentBase):
	...
	def onExecute(self, ec_id):
		...
		for object in objects_pixel:
			if not object[1] > 1413: #SSD_Detect1と被っている部分を排除
...
def SSD_Detect2Init(manager):
    profile = OpenRTM_aist.Properties(defaults_str=ssd_detect2_spec)
    manager.registerFactory(profile,
                            SSD_Detect2,
                            OpenRTM_aist.Delete)

def MyModuleInit(manager):
    SSD_Detect2Init(manager)

    # Create a component
    comp = manager.createComponent("SSD_Detect2")
